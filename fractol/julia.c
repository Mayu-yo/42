...
int julia_set_color(int iter)
{
    ...
    if (iter < (MAX_ITER/3)) {
        ...
        int green = (int)(t * 0x72 + (1 - t) * 0xf7);
        int blue = (int)(t * 0x00 + (1 - t) * 0xf0);
        return (red << 16) | (green << 8) | blue;
    } else {
        color = 0;
...
// int julia_set_color(int iter) {
//     if (iter == MAX_ITER) {
//         return 0x000000;  // Black for non-divergent points
//     } else {
//         double t = (double)iter / MAX_ITER;
//         int red = 0x00 - (t * 0x00);          // Red component
//         int green = 0xff - (t * 0xff);        // Green component
//         int blue = 0xff - (t * 0x1a);  // Blue component
//         return (red << 16) | (green << 8) | blue;
//     }
// }
...
//         while (x < WIDTH) {
//             iter = julia_set(x, y, c, zoom);
//             int pixel_color = julia_set_color(iter);
//             img->addr[y * img->line_length + (x) * (img->bits_per_pixel / 8)] = pixel_color & 0xFF; // Red
//             img->addr[y * img->line_length + (x) * (img->bits_per_pixel / 8) + 1] = (pixel_color >> 8) & 0xFF; // Green
//             img->addr[y * img->line_length + (x) * (img->bits_per_pixel / 8) + 2] = pixel_color; // Blue
//             x++;
//         }
//         y++;
//     }
